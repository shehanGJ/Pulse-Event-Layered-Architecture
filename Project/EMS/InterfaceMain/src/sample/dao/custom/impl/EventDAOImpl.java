...
import sample.dao.custom.EventDAO;
...
public class EventDAOImpl implements EventDAO {
        ...
        )
        {
            if (conn != null)
            {
                ...
                {                                            // reads a row
                    ...
                    billDTO.setMenu_cost(this.getChosenMenuCost(menu_id));
                    billDTO.setVenue_cost(this.getChosenVenueCost(venue_id));
                    billDTO.setCatering_cost(this.getChosenCateringServiceCost(catering_id));
                    billDTO.setStudio_cost(this.getChosenStudioCost(studio_id));
                    billDTO.totalCost(guests);

                    EventDTO obj = new EventDTO(name, type, date, guests, start, end, billDTO, cost);
    ...
    public ArrayList<EventDTO> getListOfEvents() {
        ...
        try (
                Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/" + GlobalsDTO.getDb_name(), GlobalsDTO.getDb_username(), GlobalsDTO.getDb_pass());
                Statement stmt = conn.createStatement();
                ResultSet rs = stmt.executeQuery(query)
        ) {
            if (conn != null) {
                ...
                while (rs.next()) {  // Process each row
                    ...
                    String studioId = rs.getString("studio_id");
                    String menuId = rs.getString("menu_id");
                    String cateringId = rs.getString("catering_id");
                    String mediaId = rs.getString("media_id");
                    int approved = rs.getInt("approved");

                    // Create a Bill object and set costs
                    BillDTO billDTO = new BillDTO();
                    billDTO.setMenu_cost(this.getChosenMenuCost(menuId));
                    billDTO.setVenue_cost(this.getChosenVenueCost(venueId));
                    billDTO.setCatering_cost(this.getChosenCateringServiceCost(cateringId));
                    billDTO.setStudio_cost(this.getChosenStudioCost(studioId));
                    billDTO.totalCost(guests);

                    // Create an Event object and populate details
                    EventDTO obj = new EventDTO(id, name, type, date, guests, start, end, billDTO);
                    obj.setApproved(approved);
                    eventDTOList.add(obj);
                }
            } else {
                System.out.println("Failed to make connection!");
            }
