...
import sample.dto.*;
import sample.util.AudioPlayer;
...
public class EventBookController {
    ...
    public void initBookEvent() {
        ...
        contact.setText("shehananujayaownme@gmail.com");
    ...
    public void initVenueVariables() {
        ...
        VenueDTO venue_DTO_obj = new VenueDTO();

        HashMap<ArrayList<String>, ArrayList<VenueDTO>> PairOfVenuesAndIDs = venue_DTO_obj.getListOfVenuesAndIDs();
        // at first index of hashmap
        ArrayList<VenueDTO> venueDTOList = (ArrayList<VenueDTO>) PairOfVenuesAndIDs.values().toArray()[0];
        venueIDs = (ArrayList<String>) PairOfVenuesAndIDs.keySet().toArray()[0];

        // picking up all three venues from database
        VenueDTO v1 = venueDTOList.get(0);
        VenueDTO v2 = venueDTOList.get(1);
        VenueDTO v3 = venueDTOList.get(2);
    ...
    public void initCatererVariables() {
        ...
        Catering_ServiceDTO catering_obj = new Catering_ServiceDTO();

        HashMap<ArrayList<String>, ArrayList<Catering_ServiceDTO>> PairOfCaterersAndIDs = catering_obj.getListOfCaterersAndIDs();
        // at first index of hashmap
        ArrayList<Catering_ServiceDTO> catererList = (ArrayList<Catering_ServiceDTO>) PairOfCaterersAndIDs.values().toArray()[0];
        catererIDs = (ArrayList<String>) PairOfCaterersAndIDs.keySet().toArray()[0];

        // picking up all three caterers from database
        Catering_ServiceDTO c1 = catererList.get(0);
        Catering_ServiceDTO c2 = catererList.get(1);
        Catering_ServiceDTO c3 = catererList.get(2);
        ...
        // rice
        menuPrices.put("biryani", 1000);
        menuPrices.put("eggfried",700);
        menuPrices.put("hopper", 300);
        menuPrices.put("Kiri Bath", 1300);
        // bread
        menuPrices.put("naan", 500);
        menuPrices.put("tandoori", 450);
        menuPrices.put("lebanese", 700);
        // meat
        menuPrices.put("chicken",1200);
        menuPrices.put("beef", 1500);
        menuPrices.put("seafood", 2200);
        menuPrices.put("mutton", 1900);
        // drinks
        menuPrices.put("miranda", 700);
        menuPrices.put("coke", 700);
        menuPrices.put("sprite", 700);
        menuPrices.put("water", 700);
        // extras
        menuPrices.put("dryfruit",700);
        menuPrices.put("icecream", 700);
        menuPrices.put("cake", 700);
    }
    ...
    public void initStudioVariables() {
        ...
        StudioDTO studio_DTO_obj = new StudioDTO();

        HashMap<ArrayList<String>, ArrayList<StudioDTO>> PairOfStudiosAndIDs = studio_DTO_obj.getListOfStudiosAndIDs();
        // at first index of hashmap
        ArrayList<StudioDTO> studioDTOList = (ArrayList<StudioDTO>) PairOfStudiosAndIDs.values().toArray()[0];
        studioIDs = (ArrayList<String>) PairOfStudiosAndIDs.keySet().toArray()[0];

        // picking up all three venues from database
        StudioDTO s1 = studioDTOList.get(0);
        StudioDTO s2 = studioDTOList.get(1);
        StudioDTO s3 = studioDTOList.get(2);
    ...
    public void generateMenu() {
        ...
        if (menu_rice_biryani.isSelected()) {
            rice += "Biryani";
            Integer biryaniPrice = menuPrices.get("biryani");
            if (biryaniPrice != null) {
                price += biryaniPrice;
            }
        }
        ...
        if (menu_rice_eggfried.isSelected()) {
            ...
            Integer eggfriedPrice = menuPrices.get("eggfried");
            if (eggfriedPrice != null) {
                price += eggfriedPrice;
            }
        }
        ...
        if (menu_rice_plain.isSelected()) {
            ...
            Integer plainPrice = menuPrices.get("plain");
            if (plainPrice != null) {
                price += plainPrice;
            }
        }
        ...
        if (menu_rice_kabuli.isSelected()) {
            ...
            Integer kabuliPrice = menuPrices.get("kabuli");
            if (kabuliPrice != null) {
                price += kabuliPrice;
            }
        }

        menuDTO.setRice(rice);

        if (menu_bread_naan.isSelected()) {
            bread += "Naan";
            Integer naanPrice = menuPrices.get("naan");
            if (naanPrice != null) {
                price += naanPrice;
            }
        }
    ...
    public void handleBookButton(ActionEvent actionEvent) throws IOException, LineUnavailableException, UnsupportedAudioFileException {
        ...
        // get event details
        eventDTO.setName(event_name.getText());
        eventDTO.setType(event_type.getText());
        eventDTO.setStarting_time(event_start.getText());
        eventDTO.setEnding_time(event_end.getText());
        eventDTO.setTotal_guests(Integer.parseInt(event_guests.getText()));
        ...
        eventDTO.setDate(temp_date);

        if (eventDTO.isDateBooked()) {
            openPopup("Warning", "That date is unavailable. Please select another.");
            return;
        }

        eventDTO.setDate(e_date);

        generateMenu();
        generateMediaReqs();

        eventDTO.bookEvent(LoggedInUsersDTO.getCust_id(), selectedVenue_id, selectedStudio_id,
               selectedMenu_id, selectedCatering_id, selectedMedia_id);

        String msg = "This is a confirmation message that your event has been booked with us. Event details are as follows: Name : " + eventDTO.getName() +
                ", Type : " + eventDTO.getType() + ", Date: " + eventDTO.getDate();
        EmailClass.sendEmail("Event Booked", msg, LoggedInUsersDTO.getCust().getEmail_address());

        goToPayment(Integer.toString(eventDTO.getPrice()));
    }
    ...
    public void goToCustMenu() throws IOException {
        ...
        //Load next
        FXMLLoader loader = new FXMLLoader(getClass().getResource("../resources/view/cust_menu.fxml"));
        Parent root = loader.load();

        //Get controller of cust bg scene
        CustMenuController controller = loader.getController();

        //setting information
        controller.setWelcome(LoggedInUsersDTO.getCust().getName());
        controller.setEventBookedStatus(LoggedInUsersDTO.getCust_id());
    ...
    public void goToPayment(String total) throws IOException {
        ...
        //Load next
        FXMLLoader loader = new FXMLLoader(getClass().getResource("../resources/view/payment.fxml"));
        Parent root = loader.load();
    ...
    public void openPopup(String heading, String text) throws IOException, LineUnavailableException, UnsupportedAudioFileException {
        //Load next
        FXMLLoader loader = new FXMLLoader(getClass().getResource("../resources/view/popup.fxml"));
        Parent root = loader.load();
