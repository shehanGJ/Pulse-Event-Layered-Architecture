...
public class CustEditController {
    ...
    public void init() {
        ...
        CustomerDTO cust = LoggedInUsersDTO.getCust();
    ...
    public void handleEditButtonAction(ActionEvent actionEvent) throws IOException, LineUnavailableException, UnsupportedAudioFileException {
        ...
        // getting id
        String id = LoggedInUsersDTO.getCust_id();
        ...
        if (!cust.getCnic().isEmpty()) {
            ...
            LoggedInUsersDTO.getCust().setCnic(cust.getCnic());
        }

        if (!cust.getName().isEmpty()) {
            cust.editCustomerAccountField(id,"name", cust.getName());
            LoggedInUsersDTO.getCust().setName(cust.getName());
        }

        if (!cust.getAccount_number().isEmpty()) {
            cust.editCustomerAccountField(id,"account_number", cust.getAccount_number());
            LoggedInUsersDTO.getCust().setAccount_number(cust.getAccount_number());
        }

        if (!c_curpass.isEmpty() && !c_newpass.isEmpty()) {
            if(cust.changePassword(id, c_curpass, c_newpass)) {
                EmailClass.sendEmail("Password Changed","Your password has been changed successfully. If this was not done by you, immediately contact us at asheventshelp@gmail.com", LoggedInUsersDTO.getCust().getEmail_address());
            }
            else {
    ...
    public void goToSignIn() throws IOException {
        ...
        CustSigninController controller = loader.getController();
    ...
    public void goToEditScreen() throws IOException {
        ...
        CustEditController controller = loader.getController();
        controller.init();
