...
public class MgrViewEventsController implements Initializable {

    /////////////// TABLE VARIABLES //////////////////

    @FXML private TableView MainTable;
    @FXML private TableColumn<EventDTO, String> event_id;
    @FXML private TableColumn<EventDTO, String> event_name;
    @FXML private TableColumn<EventDTO, String> event_type;
    @FXML private TableColumn<EventDTO, String> event_date;
    @FXML private TableColumn<EventDTO, String> event_guests;
    @FXML private TableColumn<EventDTO, String> event_price;
    @FXML private TableColumn<EventDTO, String> event_start;
    @FXML private TableColumn<EventDTO, String> event_end;
    @FXML private TableColumn<EventDTO, String> event_approved;
    ...
    private final ObservableList<EventDTO> dataList = FXCollections.observableArrayList();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        event_id.setCellValueFactory(new PropertyValueFactory<EventDTO, String>("event_id"));
        event_name.setCellValueFactory(new PropertyValueFactory<EventDTO, String>("name"));
        event_type.setCellValueFactory(new PropertyValueFactory<EventDTO, String>("type"));
        event_date.setCellValueFactory(new PropertyValueFactory<EventDTO, String>("date"));
        event_guests.setCellValueFactory(new PropertyValueFactory<EventDTO, String>("total_guests"));
        event_price.setCellValueFactory(new PropertyValueFactory<EventDTO, String>("price"));
        event_start.setCellValueFactory(new PropertyValueFactory<EventDTO, String>("starting_time"));
        event_end.setCellValueFactory(new PropertyValueFactory<EventDTO, String>("ending_time"));
        event_approved.setCellValueFactory(new PropertyValueFactory<EventDTO, String>("approved"));

        EventDTO eventDTO = new EventDTO();
        ArrayList<EventDTO> eventDTOList = eventDTO.getListOfEvents();

        for (int i = 0; i < eventDTOList.size(); i++)
            dataList.add(eventDTOList.get(i));

        loadTable();
    }

    public void setFilter(FilteredList<EventDTO> filteredData) {
    ...
    public void loadTable() {
        ...
        FilteredList<EventDTO> filteredData = new FilteredList<>(dataList, b -> true);

        // 2. Set the filter Predicate whenever the filter changes.
        setFilter(filteredData);

        // 3. Wrap the FilteredList in a SortedList.
        SortedList<EventDTO> sortedData = new SortedList<>(filteredData);
    ...
    public void handleApproveButton(ActionEvent actionEvent) throws IOException, LineUnavailableException, UnsupportedAudioFileException {
        ...
        // event is approved
        String eventID = selectedEventID.getText();
        ManagerDTO mgr = new ManagerDTO();
        mgr.approveEvent(eventID);

        // send email to customer
        EventDTO eventDTO = new EventDTO();
        String custEmail = eventDTO.getCustomerEmailByEventID(eventID);
    ...
    public void goToViewEvent(String eventID) throws IOException, LineUnavailableException, UnsupportedAudioFileException {
        ...
        //Load next
        FXMLLoader loader = new FXMLLoader(getClass().getResource("../resources/view/event_view.fxml"));
        Parent root = loader.load();
